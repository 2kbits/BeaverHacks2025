...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    ...
    if (resultsDiv)
      resultsDiv.innerHTML =
        '<p style="color: red;">Error: Page elements missing.</p>';
    return;
  }
  ...
  function populateStopSelect(selectElement, optionsArray, defaultLabel) {
    ...
    if (
      defaultLabel.includes("Loading") ||
      defaultLabel.includes("Error") ||
      defaultLabel.includes("No stops")
    ) {
    ...
    .then((response) => {
      if (!response.ok) {
        return response
          ...
          .catch(() => {
            throw new Error(
              `HTTP error ${response.status}: Failed to fetch stop names`
            );
          });
      }
      ...
      .then((data) => {
        ...
        if (data && Array.isArray(data.routes_at_stop)) {
          ...
          data.routes_at_stop.forEach((routeInfo) => {
            ...
            const avgSchedDelayText =
              routeInfo.average_scheduled_delay_at_schedule !== null
            ...
            resultsHtml += `
                <div class="result-route-box">
                  ...
                  <p><strong>Next Bus ID:</strong> ${
                    routeInfo.next_bus_id || "None found"
                  }</p>
                  <p><strong>Next Scheduled:</strong> ${
                    formatDateTime(routeInfo.next_scheduled_arrival) ||
                    "None found"
                  }</p>
          ...
          resultsDiv.innerHTML =
            "<p>Schedule data received, but format is incorrect.</p>";

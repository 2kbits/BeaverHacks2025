...
# --- Endpoint for the main chart data (Average Delay) ---
...
# --- Endpoint to provide filter options ---
...
async def get_filter_options():
    ...
    for record in raw_bus_data:
        ...
        if isinstance(hour, int): unique_hours.add(hour)
...
# --- NEW Endpoint to find specific arrival times based on filters ---
@router.get("/api/find-arrival")
async def find_arrival_time(route: str, hour: int):
    """
    Finds the first matching bus arrival record based on route and hour.
    Returns the scheduled and predicted arrival times.
    (Note: Ignores minute selection for filtering in this version).
    """
    print(f"Searching for route: {route}, hour: {hour}") # Debug log

    # Convert hour to integer just in case it comes as string from query
    try:
        target_hour = int(hour)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid hour format. Hour must be an integer.")

    for record in raw_bus_data:
        record_route = record.get("published_line")
        record_hour = record.get("hour_of_day")

        # Check if route and hour match
        if record_route == route and isinstance(record_hour, int) and record_hour == target_hour:
            scheduled = record.get("scheduled_arrival")
            predicted = record.get("predicted_arrival") # Using predicted as the "estimated" time

            # Basic check if times were found in the record
            if scheduled is not None and predicted is not None:
                print(f"Match found: Scheduled={scheduled}, Predicted={predicted}") # Debug log
                return {"scheduled": scheduled, "predicted": predicted}
            else:
                # Handle case where record matches but lacks time data (unlikely but possible)
                print("Match found but missing time data.")
                continue # Check next record

    # If no match found after checking all records
